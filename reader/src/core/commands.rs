...
/// Moved to a function with match statement to check for CardType
pub fn read_serial_number_mifare(ctx: &Context, mut readers_buf: [u8; 2048]) {
    let readers = ctx
        .list_readers(&mut readers_buf)
        .expect("failed to list readers")
        .collect::<Vec<_>>();
    ...
    let _ = match ctx.connect(readers[0], ShareMode::Shared, Protocols::ANY) {
        Ok(mut connection) => {
            ...
            let apdu = b"\xFF\xCA\x00\x00\x04";
            let mut rapdu_buf = [0; MAX_BUFFER_SIZE];
            let rapdu = tx
                .transmit(apdu, &mut rapdu_buf)
                .expect("failed to transmit APDU to card");
            let mut serial_number: Vec<u8> = Vec::new();
            println!("MIFARE Read data in 16 bytes from 0th byte: {:?}", rapdu);
            ...
            if rapdu.len() == MIFARE_SERIAL_NAME_BYTE_LENGHT {
                ...
                let data = Utf32To8(serial_number.as_ref());
                println!("Serial number MIFARE: {}", data.display_raw_string());
                for value in rapdu {
                    println!("Serial number value MIFARE: {:x}", value);
                }
            } else {
                debug!("The reading of serial name bytes has been interrupted or there are not enough bytes read.");
            }
...
/// Moved to a function with match statement to check for CardType
pub fn read_serial_number_ntag215(ctx: &Context, mut readers_buf: [u8; 2048]) {
    let readers = ctx
        .list_readers(&mut readers_buf)
        .expect("failed to list readers")
        .collect::<Vec<_>>();
    ...
    let _ = match ctx.connect(readers[0], ShareMode::Shared, Protocols::ANY) {
        Ok(mut connection) => {
            ...
            if rapdu.len() == NTAG215_SERIAL_NAME_BYTE_LENGHT {
                ...
                let data = Utf32To8(serial_number.as_ref());
                println!("Serial number NTAG: {}", data.display_raw_string());
                for value in rapdu {
                    println!("Serial number value NTAG215: {:x}", value);
                }
            } else {
                debug!("The reading of serial name bytes has been interrupted or there are not enough bytes read.");
            }
