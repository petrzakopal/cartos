...
pub struct AppState {
    ...
    pub clients: Clients,
    pub ws_body_channel_sender: tokio::sync::broadcast::Sender<WebsocketMessageBody>
}

pub fn create_routes(db_sqlite_pool: Pool<Sqlite>, ws_body_channel_sender: tokio::sync::broadcast::Sender<WebsocketMessageBody>) -> Router {
    // Init the state used in routes
    let app_state = Arc::new(AppState {
        db_sqlite_pool: db_sqlite_pool,
        clients: Clients::new(),
        ws_body_channel_sender: ws_body_channel_sender
    });
